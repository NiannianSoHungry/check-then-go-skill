...
- [ ] 是否照搬了别人的句式结构？**如果下意识想到"这是从 XX 参考的模板"，就必须改写成自己的语言**
  - 用户明确纠正："不要原模原样照抄，搞得好像我抄袭人家一样"
  - 至少 3 处关键句与参考源不同才算合格
- [ ] 是否有奇怪的口语化表达？（例："哥哥一顿改"、"哧哧一顿改"——无意义拟声词）
- [ ] 是否有内容为空的行内代码块？（检查 &lt;...&gt; 尖括号过滤问题）
- [ ] 非嵌套场景是否误用了 `~~~`？
- [ ] 展示给用户预览的文案是否放在了代码块里？（用户纠正："不要放在代码块里，直接展示"）
  - `内容直接展示`，不要 ```` ``` ```` 包裹
- [ ] Mermaid 回边是否破坏了主流程的 TD 顺序？
...
- 非嵌套场景误用 `~~~` 被纠正
- 展示文案时放在代码块里被纠正
- 直接搬运他人句式结构（"2026年了，你有Agent"）被用户纠正
...
这样用户能直接看到实际的格式效果、emoji 大小、行间距等真实样式。
...
**问题**：在 Mermaid `flowchart TD` 中，如果最后一个节点有返回边（如"判断失败 → 回到前面阶段"），Mermaid 布局引擎会为了避免回边交叉，把被循环连接的节点（如阶段二）拉到低层级，破坏 TD 的严格从上到下顺序。

**根本原因**：`G -->|是| 阶段二` 这条回边让 Mermaid 把阶段二拉到了阶段五的下方或同一行。

### 解决方案：中继节点法

用**中继节点**代替直接回边，再用虚线 `-.->` 表示实际跳转：
...
flowchart TD
    ...
    阶段五 -->|失败| 判断{重试 < 3次?}
    判断 -->|否| 回滚[回滚到检查点]
    判断 -->|是| 回到阶段二[回到阶段二 重新分析]
    回到阶段二 -.-> 阶段二
```

**要点**：
- 中继节点在失败分支的末端（"是"出口的下游）
- 虚线 `-.->` 表示跳转，不触发布局引擎的层级调整
- 阶段一到五严格保持 TD 从上到下顺序

**本会话实际违规记录**：
- 第一阶段 Mermaid 流程图中 `G -->|是| 阶段二` 导致布局混乱。
  用户纠正："因为失败分支循环的存在，这个图没有严格的遵守 TD 的顺序"
- 修复后：使用中继节点 `回到阶段二` 保持 TD 顺序，虚线连接。
